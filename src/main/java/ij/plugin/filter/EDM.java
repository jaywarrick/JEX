...
public class EDM implements ExtendedPlugInFilter {
	...
	private ImagePlus outImp; // output
	// if
	// a
	// new
	// window
	// is
	// desired
	private PlugInFilterRunner pfr; // needed
	// to
	// extract
	// the
	// stack
	...
	// showing
	// status
	private int outImageType; // output
	// type;
	// BYTE_OVERWRITE,
	// BYTE,
	// SHORT
	// or
	// FLOAT
	private ImageStack outStack; // in
	// case
	// output
	// should
	// be
	...
	// parallel
	// threads)
	private boolean interrupted; // whether
	// watershed
	// segmentation
	// has
	// been
	...
	private boolean background255; // whether
	// background
	// for
	// EDM
	// is
	...
	public void run(ImageProcessor ip)
	{
		...
		if(USES_MAX_FINDER[processType])
		{
			...
			maxIp = maxFinder.findMaxima(floatEdm, MAXFINDER_TOLERANCE, ImageProcessor.NO_THRESHOLD, maxOutputType, false, isEDM);
			if(maxIp == null)
			...
			{ // for all these, output contains the values of the EDM
				...
				case BYTE_OVERWRITE:
					...
					if(floatEdm.getMax() > 255.)
						ip.resetMinAndMax(); // otherwise we have max of
					// floatEdm
			}
	...
	public void toWatershed(ImageProcessor ip)
	{
		...
		ByteProcessor maxIp = maxFinder.findMaxima(floatEdm, MAXFINDER_TOLERANCE, ImageProcessor.NO_THRESHOLD, MaximumFinder.SEGMENTED, false, true);
		if(maxIp != null)
			ip.copyBits(maxIp, 0, 0, Blitter.AND);
	}
	
	/**
	 * Calculates a 16-bit grayscale Euclidean Distance Map for a binary 8-bit image. Each foreground (nonzero) pixel in the binary image is assigned a value equal to its distance from the nearest background (zero) pixel, multiplied by EDM.ONE. For
	 * compatibility with previous versions of ImageJ only.
	 */
	...
	public FloatProcessor makeFloatEDM(ImageProcessor ip, int backgroundValue, boolean edgesAreBackground)
	{
		...
		int nProgressUpdates = height / progressInterval; // how often the
		// progress bar is
		// updated when
		// passing once
		// through y range
