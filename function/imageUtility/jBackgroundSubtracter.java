...
/**
 * Implements ImageJ's Subtract Background command. Based on the concept of the rolling ball algorithm described in Stanley Sternberg's article, "Biomedical Image Processing", IEEE Computer, January 1983.
 * 
...
public class jBackgroundSubtracter implements ExtendedPlugInFilter, DialogListener {
	...
	public static double radius = 50; // default
	// rolling
	// ball
	// radius
	public static boolean lightBackground = Prefs.get("bs.background", true);
	private static boolean separateColors; // whether
	// to
	// create
	// a
	// separate
	...
	// color
	// channel
	public static boolean createBackground; // don't
	// subtract
	// background
	// (e.g.,
	// for
	...
	// before
	// subtracting)
	public static boolean useParaboloid; // use
	// "Sliding Paraboloid"
	// instead
	// of
	// rolling
	// ball
	// algorithm
	public static boolean doPresmooth = true; // smoothen
	// the
	// image
	// before
	// creating
	...
	private final static int MAXIMUM = 0, MEAN = 1; // filter
	// types
	// of
	// filter3x3
	private final static int X_DIRECTION = 0, Y_DIRECTION = 1, DIAGONAL_1A = 2, DIAGONAL_1B = 3, DIAGONAL_2A = 4, DIAGONAL_2B = 5; // filter
	// directions
	private final static int DIRECTION_PASSES = 9; // number
	// of
	// passes
	// for
	// different
	...
	public int showDialog(ImagePlus imp, String command, PlugInFilterRunner pfr)
	{
		...
		if((imp.getProcessor() instanceof FloatProcessor) && !createBackground)
			flags |= SNAPSHOT; // FloatProcessors need the original to subtract
	...
	{
		if(!createBackground)
			ip.setSnapshotPixels(ip.getPixels());
	 ...
	 * 
	 * @param ip
	 *            The RGB image. On output, it will become the background-subtracted image or the background (depending on <code>createBackground</code>).
	 * @param radius
	 *            Radius of the rolling ball creating the background (actually a paraboloid of rotation with the same curvature)
	 ...
	 * @param doPresmooth
	 *            Whether the image should be smoothened (3x3 mean) before creating the background. With smoothing, the background will not necessarily be below the image data.
	 * @param correctCorners
	 *            Whether the algorithm should try to detect corner particles to avoid subtracting them as a background.
	 */
	 ...
	 * 
	 * @param ip
	 *            The image. On output, it will become the background-subtracted image or the background (depending on <code>createBackground</code>).
	 * @param radius
	 *            Radius of the rolling ball creating the background (actually a paraboloid of rotation with the same curvature)
	 * @param createBackground
	 *            Whether to create a background, not to subtract it.
	...
	public void rollingBallBackground(ImageProcessor ip, double radius, boolean createBackground, boolean lightBackground, boolean useParaboloid, boolean doPresmooth, boolean correctCorners)
	{
		...
		for (int channelNumber = 0; channelNumber < ip.getNChannels(); channelNumber++)
		{
			...
			{ // subtract the background now
				float[] bgPixels = (float[]) fp.getPixels(); // currently holds
				// the background
				...
				{ // here ip and fp are the same (bgPixels will be output)
				  ...
				  // jay.show();
					...
					// for all others, the image data are in ip, the background
					// is in fp
				}
				else if(ip instanceof ShortProcessor)
				{
					float offset = invert ? 65535.5f : 0.5f;// includes 0.5 for
					// rounding when
					// converting float
					// to short
				...
				{
					float offset = invert ? 255.5f : 0.5f; // includes 0.5 for
					// rounding when
					// converting float
					// to byte
	...
	/**
	 * Create background for a float image by sliding a paraboloid over the image.
	 */
	void slidingParaboloidFloatBackground(FloatProcessor fp, float radius, boolean invert, boolean doPresmooth, boolean correctCorners)
	{
		...
		float[] cache = new float[Math.max(width, height)]; // work array for
		// lineSlideParabola
		int[] nextPoint = new int[Math.max(width, height)]; // work array for
		// lineSlideParabola
		float coeff2 = 0.5f / radius; // 2nd-order coefficient of the polynomial
		// approximating the ball
		float coeff2diag = 1.f / radius; // same for diagonal directions where
		// step is sqrt2
		
		showProgress(0.000001); // start the progress bar (only filter1D will
		// increment it)
		if(invert)
		...
		if(doPresmooth)
		{
			shiftBy = (float) filter3x3(fp, MAXIMUM);// 3x3 maximum to remove
			// dust etc.
			showProgress(0.5);
		...
		if(correctCorners)
			correctCorners(fp, coeff2, cache, nextPoint); // modify corner data,
		// avoids subtracting
		// corner particles
		
		/* Slide the parabola over the image in different directions */
		/*
		 * Doing the diagonal directions at the end is faster (diagonal lines are denser, so there are more such lines, and the algorithm gets faster with each iteration)
		 */
		filter1D(fp, X_DIRECTION, coeff2, cache, nextPoint);
		filter1D(fp, Y_DIRECTION, coeff2, cache, nextPoint);
		filter1D(fp, X_DIRECTION, coeff2, cache, nextPoint); // redo for better
		// accuracy
		...
		filter1D(fp, DIAGONAL_1A, coeff2diag, cache, nextPoint);// redo for
		// better
		// accuracy
		filter1D(fp, DIAGONAL_1B, coeff2diag, cache, nextPoint);
	...
	/**
	 * Filter by subtracting a sliding parabola for all lines in one direction, x, y or one of the two diagonal directions (diagonals are processed only for half the image per call).
	 */
	void filter1D(FloatProcessor fp, int direction, float coeff2, float[] cache, int[] nextPoint)
	{
		...
		int nLines = 0; // index+1 of the last line to handle (initialized to
		...
		switch (direction)
		{
			...
			case DIAGONAL_1A: // lines parallel to x=y, starting at x axis
				nLines = width - 2; // the algorithm makes no sense for lines
			...
			case DIAGONAL_2A: // lines parallel to x=-y, starting at x axis
				...
				pointInc = width - 1;
				break;
			case DIAGONAL_2B: // lines parallel to x=-y, starting at x=width-1,
				// y=variable
				startLine = 0;
				nLines = height - 2;
				lineInc = width;
	...
	/**
	 * Process one straight line in the image by sliding a parabola along the line (from the bottom) and setting the values to make all points reachable by the parabola
	 * 
	 * @param pixels
	 *            Image data, will be modified by parabolic interpolation where the parabola does not touch.
	 * @param start
	 ...
	 * @param coeff2
	 *            2nd order coefficient of the polynomial describing the parabola, must be positive (although a parabola with negative curvature is actually used)
	 * @param cache
	 *            Work array, length at least <code>length</code>. Will usually remain in the CPU cache and may therefore speed up the code.
	 * @param nextPoint
	 *            Work array. Will hold the index of the next point with sufficient local curvature to get touched by the parabola.
	 * @param correctedEdges
	 *            Should be a 2-element array used for output or null.
	 * @return The correctedEdges array (if non-null on input) with the two estimated edge pixel values corrected for edge particles.
	 */
	static float[] lineSlideParabola(float[] pixels, int start, int inc, int length, float coeff2, float[] cache, int[] nextPoint, float[] correctedEdges)
	{
		...
		int lastpoint = 0;
		int firstCorner = length - 1; // the first point except the edge that is
		// touched
		int lastCorner = 0; // the last point except the edge that is touched
		float vPrevious1 = 0f;
		float vPrevious2 = 0f;
		float curvatureTest = 1.999f * coeff2; // not 2: numeric scatter of 2nd
		// derivative
		/*
		 * copy data to cache, determine the minimum, and find points with local curvature such that the parabola can touch them - only these need to be examined futher on
		 */
		...
		int i1 = 0; // i1 and i2 will be the two points where the parabola
		// touches
		while (i1 < length - 1)
		{
			float v1 = cache[i1];
			...
			int searchTo = length;
			int recalculateLimitNow = 0; // when 0, limits for searching will be
			// recalculated
			/*
			...
			for (int j = nextPoint[i1]; j < searchTo; j = nextPoint[j], recalculateLimitNow++)
			{
				...
				{ // time-consuming recalculation of search limit: wait a bit
				  // after slope is updated
		...
		/*
		 * Now calculate estimated edge values without an edge particle, allowing for vignetting described as a 6th-order polynomial:
		 */
		if(correctedEdges != null)
		{
			...
			float slope = (v2 - v1) / (lastCorner - firstCorner); // of the line
			// through the
			// two
			// outermost
			// non-edge
			...
			{// compare with mid-image pixels to detect vignetting
				float dx = (i - mid) * 2f / (lastCorner - firstCorner);
				float poly6 = dx * dx * dx * dx * dx * dx - 1f; // the 6th order
				// polynomial,
				// zero at
				// firstCorner
				// and
	...
	/**
	 * Detect corner particles and adjust corner pixels if a particle is there. Analyzing the directions parallel to the edges and the diagonals, we average over the 3 correction values (found for the 3 directions)
	 */
	void correctCorners(FloatProcessor fp, float coeff2, float[] cache, int[] nextPoint)
	{
		...
		float[] corners = new float[4]; // (0,0); (xmax,0); (ymax,0);
		// (xmax,ymax)
		...
		corners[3] += correctedEdges[1];
		int diagLength = Math.min(width, height); // length of a 45-degree line
		// from a corner
	...
	FloatProcessor shrinkImage(FloatProcessor ip, int shrinkFactor)
	{
		...
		for (int ySmall = 0; ySmall < sHeight; ySmall++)
		{
			for (int xSmall = 0; xSmall < sWidth; xSmall++)
			{
				...
				sPixels[xSmall + ySmall * sWidth] = min; // each point in small
				// image is minimum of
				// its neighborhood
			}
		}
	...
	/**
	 * 'Rolls' a filtering object over a (shrunken) image in order to find the image's smooth continuous background. For the purpose of explaining this algorithm, imagine that the 2D grayscale image has a third (height) dimension defined by the
	 * intensity value at every point in the image. The center of the filtering object, a patch from the top of a sphere having radius BallRadius, is moved along each scan line of the image so that the patch is tangent to the image at one or more
	 * points with every other point on the patch below the corresponding (x,y) point of the image. Any point either on or below the patch during this process is considered part of the background. Shrinking the image before running this procedure is
	 * advised for large ball radii because the processing time increases with ball radius^2.
	 */
	void rollBall(RollingBall ball, FloatProcessor fp)
	{
		...
		float[] cache = new float[width * ballWidth]; // temporarily stores the
		// pixels we work on
		...
		{ // for all positions of the ball center:
			...
			int nextLineToRead = y + radius; // line of the input not touched
			// yet
			if(nextLineToRead < height)
			{
				...
				for (int x = 0, p = nextLineToRead * width; x < width; x++, p++)
					pixels[p] = -Float.MAX_VALUE; // unprocessed pixels start at
				// minus infinity
			}
			int y0 = y - radius; // the first line to see whether the ball
			// touches
			if(y0 < 0)
				y0 = 0;
			int yBall0 = y0 - y + radius; // y coordinate in the ball
			// corresponding to y0
			int yend = y + radius; // the last line to see whether the ball
			// touches
			if(yend >= height)
				yend = height - 1;
			for (int x = -radius; x < width + radius; x++)
			{
				float z = Float.MAX_VALUE; // the height of the ball (ball is in
				// position x,y)
				int x0 = x - radius;
				if(x0 < 0)
					x0 = 0;
				...
				{ // for all points inside the ball
					...
					for (int xp = x0, bp = xBall0 + yBall * ballWidth; xp <= xend; xp++, cachePointer++, bp++)
					{
						...
						if(z > zReduced) // does this point imply a greater
							// height?
							z = zReduced;
					}
				}
				...
				// if (x>=0&&y>=0&&x<width&&y<height) bgPixels[x+y*width] = z;
				// //debug, ball height output
			}
		}
	...
	/**
	 * Uses bilinear interpolation to find the points in the full-scale background given the points from the shrunken image background. (At the edges, it is actually extrapolation.)
	 */
	void enlargeImage(FloatProcessor smallImage, FloatProcessor fp, int shrinkFactor)
	{
		...
		float[] pixels = (float[]) fp.getPixels();
		float[] sPixels = (float[]) smallImage.getPixels();
		int[] xSmallIndices = new int[width]; // index of first point in
		// smallImage
		...
		for (int y = 0; y < height; y++)
		{
			if(ySmallLine0 < ySmallIndices[y])
			{
				...
				ySmallLine0++;
				int sYPointer = (ySmallIndices[y] + 1) * smallWidth; // points
				// to line0
				// + 1 in
				// smallImage
				for (int x = 0; x < width; x++)
	...
	void makeInterpolationArrays(int[] smallIndices, float[] weights, int length, int smallLength, int shrinkFactor)
	{
		for (int i = 0; i < length; i++)
		{
			...
			float distance = (i + 0.5f) / shrinkFactor - (smallIndex + 0.5f); // distance
			// of
			// pixel
			// centers
			// (in
	...
	/**
	 * Replace the pixels by the mean or maximum in a 3x3 neighborhood. No snapshot is required (less memory needed than e.g., fp.smooth()). When used as maximum filter, it returns the average change of the pixel value by this operation
	 */
...
/**
 * A rolling ball (or actually a square part thereof) Here it is also determined whether to shrink the image
 */
class RollingBall {
	...
	/**
	 * Computes the location of each point on the rolling ball patch relative to the center of the sphere containing it. The patch is located in the top half of this sphere. The vertical axis of the sphere passes through the center of the patch. The
	 * projection of the patch in the xy-plane below is a square.
	 */
	void buildRollingBall(double ballradius, int arcTrimPer)
	{
		...
		int xval, yval; // x,y-values on patch relative to center of rolling
		// ball
		double smallballradius; // radius of rolling ball (downscaled in x,y and
		// z when image is shrunk)
		int halfWidth; // distance in x or y from center of patch to any edge
		// (patch "radius")
		...
		xtrim = (int) (arcTrimPer * smallballradius) / 100; // only use a patch
		// of the rolling
		// ball
		...
		for (int y = 0, p = 0; y < width; y++)
			for (int x = 0; x < width; x++, p++)
			{
				...
				// -Float.MAX_VALUE might be better than 0f, but gives different
