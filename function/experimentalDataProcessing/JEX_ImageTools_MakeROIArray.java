 ...
 * 
 */
public class JEX_ImageTools_MakeROIArray extends ExperimentalDataCrunch {
	
	public JEX_ImageTools_MakeROIArray()
	{}
	...
	public ParameterSet requiredParameters()
	{
		...
		Parameter p7 = new Parameter("Staggered Pattern?", "Should pattern be staggered (close-packed)?", Parameter.DROPDOWN, new String[] { "True", "False" }, 1);
		Parameter p8 = new Parameter("First Row Is Staggered?", "Is the first row inset from the left in the staggered patter (ignored if not a staggered pattern)", Parameter.DROPDOWN, new String[] { "True", "False" }, 1);
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		if(rows < 2 && cols < 2)
		{
			Logs.log("Can't make an array with less than two locations!", 9, this);
		...
		for (DimensionMap roiM : roiMap.keySet())
		{
			...
			// Map<DimensionMap, Point> pointMap = getArrayOfPoints(rows, cols,
			// staggered, staggeredFirst, corners, rowName, colName);
			// if(pointMap == null)
			// {
			// Logs.log("Couldn't make the array of points!",
			// 9, this);
			...
			if(pattern == null)
			{
				Logs.log("Couldn't make the array of points!", 9, this);
				JEXStatics.statusBar.setStatusText("Couldn't make the array of points!");
				return false;
			}
	...
	{
		if(corners.size() < 3 || rows < 1 || cols < 1)
			return null;
