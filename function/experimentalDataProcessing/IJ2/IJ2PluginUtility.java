...
public class IJ2PluginUtility {
	...
	public static TreeMap<String,IJ2Plugin> ijCommands = getIJ2Commands();
	...
	public static TreeMap<String,IJ2Plugin> getIJ2Commands()
	{
		List<CommandInfo> commands = ij.command().getCommands();
		TreeMap<String,IJ2Plugin> commandTree = new TreeMap<String,IJ2Plugin>();
		for (final CommandInfo command : commands)
		{
			if(isValidForJEX(command))
			{
				...
				if(p != null)
				{
					Logs.log("Successfully incorporated ImageJ Plugin: " + command.getTitle(), IJ2Plugin.class);
