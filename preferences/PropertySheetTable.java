...
public class PropertySheetTable extends JTable {
	 ...
	 *             if the current editor factory is not a PropertyEditorRegistry
	 */
	@Deprecated
	 ...
	 * @throws ClassCastException
	 *             if the current renderer factory is not a PropertyRendererRegistry
	 */
	@Deprecated
	 ...
	 * 
	 * @param type
	 *            the type for which a renderer should be found
	...
	/**
	 * By default, properties with children are painted with the same indent level as other properties and categories. When nested properties exist within the set of properties, the end-user might be confused by the category and property handles.
	 * Sets this property to true to add an extra indent level to properties.
	 * 
	...
	/**
	 * Ensures the table uses the full height of its parent {@link javax.swing.JViewport}.
	 */
	@Override
	...
	private static class CellBorder implements Border {
		...
		public void paintBorder(Component c, Graphics g, int x, int y, int width, int height)
		{
			...
			if(showToggle)
			{
				...
				drawIcon.paintIcon(c, g, x + indentWidth + (HOTSPOT_SIZE - 2 - drawIcon.getIconWidth()) / 2, y + (height - drawIcon.getIconHeight()) / 2);
			}
		}
	...
	private static class NameRenderer extends DefaultTableCellRenderer {
		...
		public NameRenderer()
		{
			this(UIManager.getColor(PANEL_BACKGROUND_COLOR_KEY), UIManager.getColor(PANEL_BACKGROUND_COLOR_KEY).darker());
		}
	...
	private static class CategoryValueRenderer extends DefaultTableCellRenderer {
		...
		{
			this(UIManager.getColor(PANEL_BACKGROUND_COLOR_KEY), UIManager.getColor(PANEL_BACKGROUND_COLOR_KEY).darker().darker().darker());
		}
