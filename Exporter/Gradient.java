...
public class Gradient {
	...
	/**
	 * Creates an array of Color objects for use as a gradient, using an array of Color objects. It uses a linear interpolation between each pair of points. The parameter numSteps defines the total number of colors in the returned array, not the
	...
	/**
	 * Creates a gradient from red to green with a black transition at the index INDEXOFMEAN If INDEXOFMEAN is <= 0, then the red section will not show If INDEXOFMEAN is >= 0, then the green section will not show
	 * 
	 * @param numSteps
	 * @param indexOfMean
	 * @return
	...
	public static Color[] createRedToGreenGradientWithSetMean(final int numSteps, int indexOfMean)
	{
		...
		// If the index of the black transition is less then 0 then do not show
		// the red
		...
		// If the index of the black transition is more than the max, then do
		// not show the green
		...
		if(indexOfMean >= numSteps)
			indexOfMean = numSteps;
		...
		{
			iNorm = (i - indexOfMean) / (double) (numSteps - indexOfMean); // a
			// normalized
			// [0:1]
			// variable
