 ...
 * in width or height version 01-Nov-2009 Bugfix: extra lines in segmented output eliminated; watershed is also faster now Maximum points encoded in long array for sorting instead of separete objects that need gc New output type 'List'
 */
...
public class MaximumFinder implements ExtendedPlugInFilter, DialogListener {
	...
	private ImagePlus imp; // the
	// ImagePlus
	// of
	// the
	// setup
	// call
	private int flags = DOES_ALL | NO_CHANGES | NO_UNDO; // the
	// flags
	// (see
	// interfaces
	// PlugInFilter
	...
	private boolean thresholded; // whether
	// the
	// input
	// image
	// has
	// a
	// threshold
	private boolean roiSaved; // whether
	// the
	// filter
	// has
	// changed
	...
	private boolean thresholdWarningShown = false; // whether
	// the
	// warning
	// "can't find minima with thresholding"
	// has
	// been
	// shown
	private Label messageArea; // reference
	// to
	// the
	// textmessage
	// area
	...
	// of
	// maxima
	private boolean noPointLabels; // save
	// this
	// setting
	// so
	// it
	...
	// on
	// exit
	private double progressDone; // for
	// progress
	// bar,
	// fraction
	// of
	...
	// dimensions
	private int intEncodeXMask; // needed
	// for
	// encoding
	// x
	// &
	...
	// for
	// x
	private int intEncodeYMask; // needed
	// for
	// encoding
	// x
	// &
	...
	// for
	// y
	private int intEncodeShift; // needed
	// for
	// encoding
	// x
	// &
	 ...
	 */
	final static byte MAXIMUM = (byte) 1; // marks
	// local
	// maxima
	// (irrespective
	// of
	// noise
	// tolerance)
	final static byte LISTED = (byte) 2; // marks
	// points
	// currently
	// in
	// the
	// list
	final static byte PROCESSED = (byte) 4; // marks
	// points
	// processed
	// previously
	final static byte MAX_AREA = (byte) 8; // marks
	// areas
	// near
	// a
	// maximum,
	...
	// the
	// tolerance
	final static byte EQUAL = (byte) 16; // marks
	// contigous
	// maximum
	// points
	// of
	// equal
	// level
	final static byte MAX_POINT = (byte) 32; // marks
	// a
	// single
	// point
	// standing
	...
	// a
	// maximum
	final static byte ELIMINATED = (byte) 64; // marks
	// maxima
	// that
	// have
	// been
	 ...
	 * @return Code describing supported formats etc. (see ij.plugin.filter.PlugInFilter & ExtendedPlugInFilter)
	 */
	...
	public int showDialog(ImagePlus imp, String command, PlugInFilterRunner pfr)
	{
		...
		ip.resetBinaryThreshold(); // remove invisible threshold set by
		// MakeBinary and Convert to Mask
		...
		if(useMinThreshold && ((invertedLut && !lightBackground) || (!invertedLut && lightBackground)))
		{
			if(!thresholdWarningShown)
				if(!IJ.showMessageWithCancel("Find Maxima", "\"Above Lower Threshold\" option cannot be used\n" + "when finding minima (image with light background\n" + "or image with dark background and inverting LUT).") && !previewing)
					return false; // if faulty input is not detected during
			// preview, "cancel" quits
			thresholdWarningShown = true;
			useMinThreshold = false;
			// ((Checkbox)(checkboxes.elementAt(1))).setState(false); //reset
			// "Above Lower Threshold" checkbox
		}
		if(!gd.getPreviewCheckbox().getState())
			messageArea.setText(""); // no "nnn Maxima" message when not
		// previewing
	...
	public ByteProcessor findMaxima(ImageProcessor ip, double tolerance, double threshold, int outputType, boolean excludeOnEdges, boolean isEDM)
	{
		...
		// to
		// calibrated
		ByteProcessor typeP = new ByteProcessor(width, height); // will be a
		// notepad for
		// pixel types
		...
		if(outputType == SEGMENTED)
		{
			...
			cleanupMaxima(outIp, typeP, maxPoints); // eliminate all the small
			// maxima (i.e. those
			// outside MAX_AREA)
			...
			if(!isEDM)
				cleanupExtraLines(outIp); // eliminate lines due to local minima
			// (none in EDM)
	...
	long[] getSortedMaxPoints(ImageProcessor ip, ByteProcessor typeP, boolean excludeEdgesNow, boolean isEDM, float globalMin, float globalMax, double threshold)
	{
		...
		{ // find local maxima now
			...
			{ // for better performance with rois, restrict search to roi
				float v = ip.getPixelValue(x, y);
				float vTrue = isEDM ? trueEdmHeight(x, y, ip) : v; // for EDMs,
				// use
				// interpolated
				// ridge
				// height
	...
	void analyzeAndMarkMaxima(ImageProcessor ip, ByteProcessor typeP, long[] maxPoints, boolean excludeEdgesNow, boolean isEDM, float globalMin, double tolerance, int outputType)
	{
		...
		int[] pList = new int[width * height]; // here we enter points starting
		// from a maximum
		...
		{ // process all maxima now, starting from the highest
			...
			if((types[offset0] & PROCESSED) != 0) // this maximum has been
				...
				{ // analyze all neighbors (in 8 directions) at the same level
					...
					if((isInner || isWithin(x, y, d)) && (types[offset2] & LISTED) == 0)
					{
						if((types[offset2] & PROCESSED) != 0)
						{
							...
							// processed previously, thus
							// it is no maximum now
						...
						if(isEDM && (v2 <= v - (float) tolerance))
							v2 = trueEdmHeight(x2, y2, ip); // correcting for
						// EDM peaks may
						// move the point up
						if(v2 > v)
						{
							maxPossible = false; // we have reached a higher
							// point, thus it is no maximum
						...
						else if(v2 >= v - (float) tolerance)
						{
							...
							listLen++; // we have found a new point within the
							// tolerance
							...
							if(v2 == v)
							{ // prepare finding center of equal points (in case
							  // single point needed)
								types[offset2] |= EQUAL;
								xEqual += x2;
								yEqual += y2;
	...
	void cleanupExtraLines(ImageProcessor ip)
	{
		...
		for (int y = 0, i = 0; y < height; y++)
		{
			for (int x = 0; x < width; x++, i++)
			{
				...
				if(v != (byte) 255 && v != 0)
				{
					int nRadii = nRadii(pixels, x, y); // number of lines
					// radiating
				...
				if(isInner || isWithin(x, y, d))
				{
					...
					if(v != (byte) 255 && v != 0)
					{
						...
						{ // found a point or line end
							x += DIR_X_OFFSET[d];
							y += DIR_Y_OFFSET[d];
							pixels[x + width * y] = (byte) 255; // delete the
							// point
