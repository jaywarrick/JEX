...
/**
 * An implementation of a PropertySheet which shows a table to edit/view values, a description pane which updates when the selection changes and buttons to toggle between a flat view and a by-category view of the properties. A button in the toolbar
 * allows to sort the properties and categories by name.
 * <p>
 * Default sorting is by name (case-insensitive). Custom sorting can be implemented through {@link com.l2fprod.common.propertysheet.PropertySheetTableModel#setCategorySortingComparator(Comparator)} and
...
public class PropertySheetPanel extends JPanel implements PropertySheet, PropertyChangeListener {
	 ...
	 * 
	 * Note: listeners previously added with {@link PropertySheetPanel#addPropertySheetChangeListener(PropertyChangeListener)} must be re-added after this call if the table model is not the same as the previous table.
	 * 
	 * @param table
	 */
