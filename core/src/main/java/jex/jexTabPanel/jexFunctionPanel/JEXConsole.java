 ...
 * Install via:
 * 
 * PrintStream con=new PrintStream(new JEXConsole(...));
 * System.setOut(con);
 * System.setErr(con);
 * 
 * @author Lawrence Dol
...
public JEXConsole(int maxlin) {
    if(maxlin<1) { throw new IllegalArgumentException("TextAreaOutputStream maximum lines must be positive (value="+maxlin+")"); }
    oneByte=new byte[1];
    appender=new Appender(this.panel,maxlin);
    this.innerPanel.setLayout(new MigLayout("flowy, ins 3", "[fill,grow]", "[]"));
    this.innerPanel.add(this.panel, "growx");
