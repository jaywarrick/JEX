 ...
 * 
 *         Modified by Michel Boudinot (mb) 24 Aug 2006. michel.boudinot at inaf.cnrs-gif.fr - improved decoding strategy, more robust against zvi file structure diversity. - new approach to compute the number of channels. - new image stack
 *         construction from file image slices.
 */
...
public class ZVI_Reader {
	...
	// + (mb)
	// + new code
	private Set<Integer> C_Set = new HashSet<Integer>(); // to
	// hold
	// C
	// channel
	// index
	...
	public Hashtable<Integer,ImageStack> decodeZVI(String zviFileName)
	{
		...
		Logs.log("Opening: " + directory + this.fileName, 1, this);
		FileInfo[] fi = null;
		try
		{
			fi = this.getHeaderInfo(directory, this.fileName);
		}
		catch (Exception e)
		{
			Logs.log("ZVI Reader error", 1, this);
			return null;
		}
		if(fi == null)
		{
			Logs.log("Could not find header information", 1, this);
			return null;
		}
		...
		if(imp == null)
		{
			Logs.log("Could not extract pixel data.", 1, this);
			return null;
		}
		...
		Iterator<Integer> it = this.C_Set.iterator();
		while (it.hasNext())
		{
			Object theC = it.next();
		...
		for (int n = 0; n < nSlice; n++)
		{
			...
			this.ip1 = istk.getProcessor(n + 1);
			img.get(new Integer(theC)).addSlice(null, this.ip1);
		}
	...
	private FileInfo[] getHeaderInfo(String directory, String fileName) throws IOException
	{
		...
		// + (mb)
		// + new code
		Set<Integer> Z_Set = new HashSet<Integer>(); // to hold Z plan index
		// collection.
		Set<Integer> T_Set = new HashSet<Integer>(); // to hold T time index
		// collection.
		// - (mb)
		long pos = 0;
		...
		while (true)
		{
			...
			long header = this.findBlock(in, ZVI_MAGIC_BLOCK_1, pos);
			...
			this.C_Set.add(new Integer(theC));
		...
		numZ = Z_Set.size();
		this.numC = this.C_Set.size();
		numT = T_Set.size();
		// - (mb)
		
		if(numZ * this.numC * numT != blockList.size())
		{
			Logs.log("Warning: image counts do not match.", 1, this);
		}
		...
		{
			ZVIBlock zviBlock = blockList.elementAt(i);
			int ft = -1;
			...
			if(ft < 0)
			{
				Logs.log("Warning: unknown file type for image plane #" + (i + 1), 1, this);
	...
	/**
	 * Finds the first occurrence of the given byte block within the file, starting from the given file position.
	 */
	...
	private class ZVIBlock {
		...
		public ZVIBlock(int theZ, int theC, int theT, int width, int height, int alwaysOne, int bytesPerPixel, int pixelType, int bitDepth, long imagePos)
		{
			...
			{
				Logs.log("Warning: incompatible bytesPerPixel (" + bytesPerPixel + ") and numChannels (" + this.numChannels + "). Assuming grayscale data.", 1, this);
				this.numChannels = 1;
			}
