 ...
 * 
 */
public class JEX_MakeFACSLogRates extends JEXCrunchable {
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		for (DimensionMap fileMap : fileTable.getMapIterator())
		{
			Table<Double> timePoint = JEXTableReader.getNumericTable(fileTreeMap.get(fileMap));
			if(trackDim == null)
			{
			...
			for (int t = 0; t < xMaps.size(); t++)
			{
				...
				{
					Logs.log("Whoa there. Track numbers aren't matching.", 0, this);
					return false;
				}
	...
	public Pair<DimensionMap,String> writeLogRatesTimePoint(DataWindows tracks, Dim trackDim, Dim timeDim, String timeFormat, Double tInt)
	{
		...
		String tablePath = JEXTableWriter.writeTable("LogRateData", table);
