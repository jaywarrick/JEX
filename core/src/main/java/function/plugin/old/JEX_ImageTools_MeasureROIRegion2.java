...
public class JEX_ImageTools_MeasureROIRegion2 extends JEXCrunchable {
	...
	public boolean run(JEXEntry entry, HashMap<String,JEXData> inputs)
	{
		...
		int count = 0;
		int percentage = 0;
		int total = unionTable.mapCount();

		// Get a dim name and hold info from a roi pattern if necessary
		String patternDimName = this.getNextDimName("ROI", roiTable);
		boolean hasPattern = false, atLeastOneHasPattern = false;
		int maxPointId = -1;
		int minPointId = Integer.MAX_VALUE;
		for(DimensionMap imageMap : imageData.getDimTable().getMapIterator())
		{
			...
			for(DimensionMap unionMap : unionTable.getMapIterator(imageMap))
			{
				...
				PatternRoiIterator itr = baseRoi.patternRoiIterator();
				while (itr.hasNext())
				{
					roi = itr.next();
					...
					DimensionMap newMap = unionMap.copy();
					if(hasPattern)
					{
						atLeastOneHasPattern = true;
						int id = itr.currentPatternPoint().id;
						maxPointId = Math.max(maxPointId, id);
						minPointId = Math.min(minPointId, id);
						newMap.put(patternDimName, "" + id);
					}

					this.writeData(newMap, resultsTreeMap, measure, stats, center, im, baseRoi);
				}
	...
	private void writeData(DimensionMap newMap, TreeMap<DimensionMap,Double> resultsTreeMap, String measure, ImageStatistics stats, IdPoint center, ImagePlus im, ROIPlus roi)
	{
