...
@Plugin(
		...
		name="Copy Object To Other Entries",
...
public class CopyObjectToOtherEntries extends JEXPlugin {

	public CopyObjectToOtherEntries()
	{}
	...
	{		
		...
		{
			JEXDialog.messageDialog("Support for copying File and Image objects is only available at this time. Aborting.", this);
			return false;
		}
		if(myData == null)
		{
	...
	public void finalizeTicket(Ticket ticket)
	{
		...
		TreeMap<DimensionMap,Integer> lut = getLookUpTable("UL", rows, cols, true, true);
		...
		for (Entry<Integer,JEXEntry> e : ilut.entrySet())
		{
			...
			for(DimensionMap mapToGet : dt.getMapIterator())
			{
			...
			JEXData temp = null;
			if(theData.getTypeName().getType().matches(JEXData.IMAGE))
			{
