...
@Plugin(
		...
		name="Super Fast Adaptive Median Filter",
		menuPath="Image Processing",
		visible=true,
		description="Fast median filter that uses a square shaped kernal and random sampling of the region to reduce calc time. It then adaptively converges to a weighted median for each pixel."
		)
public class SuperFastAdaptiveMedianFilter extends JEXPlugin {

	public SuperFastAdaptiveMedianFilter()
	{}
	...
	@ParameterMarker(uiOrder=3, name="Iterations", description="Size of the random sample within the kernal for calculating the median.", ui=MarkerConstants.UI_TEXTFIELD, defaultText="6")
	int iters;
	
	@ParameterMarker(uiOrder=4, name="Scaling of Weights", description="Weigths are given by abs(x-med)^(scaling). So, scaling impacts which numbers are weighted more. A scaling of 0.25-0.1 is generally optimal (0.2 recommended). This gives more weight to values nearer the current median.", ui=MarkerConstants.UI_TEXTFIELD, defaultText="5.0")
	double scaling;
	
	@ParameterMarker(uiOrder=5, name="Resample Each Location?", description="If checked, this will regenerate random locations within each neighborhood at each pixel location, incurring time lost to generate random numbers.", ui=MarkerConstants.UI_CHECKBOX, defaultBoolean=true)
	boolean resample;
	...
	public < T extends RealType< T >> ImagePlus getMedianBackground(String imagePath, int width, int sampleSize, boolean performSubtraction, int outputBitDepth, boolean resample)
	{
		...
		for ( final Neighborhood< T > localNeighborhood : shape.neighborhoods( source ) )
		{
			...
			if(!resample)
			{
				...
				median = StatisticsUtility.adaptiveMedian(vals, iters, scaling);
			}
			else
			{
