...
public class DimpledGaussianBlur {
...
//    private boolean calledAsPlugin;
	...
	*/
	public void blur(ImageProcessor ip, double sigma, double outerWeighting) {
		double accuracy = (ip instanceof ByteProcessor||ip instanceof ColorProcessor)?0.002:0.0002;
		blur(ip, sigma, sigma, outerWeighting, accuracy);
	}
     ...
     */
    public void blur(ImageProcessor ip, double sigmaX, double sigmaY, double outerWeighting, double accuracy) {
        ...
        for (int i=0; i<ip.getNChannels(); i++) {
            ...
            blurFloat(fp, sigmaX, sigmaY, outerWeighting, accuracy);
     ...
     *                  accuracy needs slightly more computing time.
     */
    public void blurFloat(FloatProcessor ip, double sigmaX, double sigmaY, double outerWeighting, double accuracy) {
        if (sigmaX > 0)
            blur1Direction(ip, sigmaX, outerWeighting, accuracy, true, (int)Math.ceil(5*sigmaY));
        if (Thread.currentThread().isInterrupted()) return; // interruption for new parameters during preview?
        if (sigmaY > 0)
            blur1Direction(ip, sigmaY, outerWeighting, accuracy, false, 0);
        return;
    }
     ...
     */
    public void blur1Direction( final FloatProcessor ip, final double sigma, final double outerWeighting, final double accuracy,
        ...
        final int maxLength = doDownscaling ?
                ...
                : length;
        final float[][] gaussKernel = makeDimpledGaussianKernel(sigmaGauss, outerWeighting, accuracy, maxLength);
...
//        showProgress(1.0);
     ...
     * @param outerWeighting	How much to weight the outer portion of the Gaussian relative
     * 				   to the inner portion (dips in middle). 0 produces regular Gaussian
     ...
     */
    public float[][] makeDimpledGaussianKernel(final double sigma, double outerWeighting, double accuracy, int maxRadius) {
        ...
        for (int i=0; i<kRadius; i++)               // Gaussian function
            kernel[0][i] = (float)(Math.exp(-0.5*i*i/sigma/sigma)*Math.pow(1.0-Math.exp(-0.5*i*i/sigma/sigma), outerWeighting));
        ...
        {
        		sum = sigma*Math.sqrt(2d*Math.PI);
        }
//        {
//        		double B = 0;
//        		sum = Math.sqrt(2d*Math.PI)*(-(2d/Math.sqrt(1d/B/B+1d/sigma/sigma))+(1/Math.sqrt(2d/B/B+1/sigma/sigma))+(1d/Math.sqrt(1d/sigma/sigma)));
//        }
