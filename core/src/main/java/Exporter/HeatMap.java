 ...
 * </p>
 * <p>
 * For specifying data, the first index into the double[][] array is the x- coordinate, and the second index is the y-coordinate. In the constructor and updateData method, the 'useGraphicsYAxis' parameter is used to control whether the row y=0 is
 * displayed at the top or bottom. Since the usual graphics coordinate system has y=0 at the top, setting this parameter to true will draw the y=0 row at the top, and setting the parameter to false will draw the y=0 row at the bottom, like in a
 * regular, mathematical coordinate system. This parameter was added as a solution to the problem of "Which coordinate system should we use? Graphics, or mathematical?", and allows the user to choose either coordinate system. Because the HeatMap will
 * be plotting the data in a graphical manner, using the Java Swing framework that uses the standard computer graphics coordinate system, the user's data is stored internally with the y=0 row at the top.
 * </p>
 * <p>
...
public class HeatMap extends JPanel {
	 ...
	 *            If true, the data will be displayed with the y=0 row at the top of the screen. If false, the data will be displayed with they=0 row at the bottom of the screen.
	 * @param colors
	 *            A variable of the type Color[]. See also {@link #createMultiGradient} and {@link #createGradient}.
	 */
	...
	/**
	 * Updates the gradient used to display the data. Calls drawData() and repaint() when finished.
	 * 
	 * @param colors
	...
	/**
	 * This uses the current array of colors that make up the gradient, and assigns a color index to each data point, stored in the dataColorIndices array, which is used by the drawData() method to plot the points.
	 */
	...
	/**
	 * This function generates data that is not vertically-symmetric, which makes it very useful for testing which type of vertical axis is being used to plot the data. If the graphics Y-axis is used, then the lowest values should be displayed at the
	 * top of the frame. If the non-graphics (mathematical coordinate-system) Y-axis is used, then the lowest values should be displayed at the bottom of the frame.
	 * 
	...
	/**
	 * This function generates an appropriate data array for display. It uses the function: z = sin(x)*cos(y). The parameter specifies the number of data points in each direction, producing a square matrix.
	 * 
	 * @param dimension
	...
	/**
	 * This function generates an appropriate data array for display. It uses the function: z = Math.cos(Math.abs(sX) + Math.abs(sY)). The parameter specifies the number of data points in each direction, producing a square matrix.
	 * 
	 * @param dimension
	 *            Size of each side of the returned array
	 ...
	 * 
	 * After doing some profiling, it was discovered that 90% of the drawing time was spend drawing the actual data (not on the axes or tick marks). Since the Graphics2D has a drawImage method that can do scaling, we are using that instead of scaling
	 * it ourselves. We only need to draw the data into the bufferedImage on startup, or if the data or gradient changes. This saves us an enormous amount of time. Thanks to Josh Hayes-Sheen (grey@grevian.org) for the suggestion and initial code to
	 * use the BufferedImage technique.
	 * 
	 * Since the scaling of the data plot will be handled by the drawImage in paintComponent, we take the easy way out and draw our bufferedImage with 1 pixel per data point. Too bad there isn't a setPixel method in the Graphics2D class, it seems a
	 * bit silly to fill a rectangle just to set a single pixel...
	 * 
	...
	/**
	 * The overridden painting method, now optimized to simply draw the data plot to the screen, letting the drawImage method do the resizing. This saves an extreme amount of time.
	 */
	@Override
